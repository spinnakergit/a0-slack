#!/bin/bash
# Install the Slack Integration plugin into an Agent Zero instance.
#
# Usage:
...
PLUGIN_DIR="$A0_ROOT/usr/plugins/slack"

echo "=== Slack Integration Plugin Installer ==="
...
if [ "$A0_ROOT" = "/a0" ] && [ -d "/git/agent-zero/usr" ]; then
    GIT_PLUGIN="/git/agent-zero/usr/plugins/slack"
...
echo ""
echo "Next steps:"
echo "  1. Configure your bot token in the Slack plugin settings (WebUI)"
echo "     Or set SLACK_BOT_TOKEN environment variable"
echo "  2. Restart Agent Zero to load the plugin"
echo "  3. Ask the agent: 'List Slack channels'"
